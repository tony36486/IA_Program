...
{
	// Read input image
	cv::Mat image= cv::imread("mumu1.jpg",0);
	if (!image.data)
		return 0; 
	...
	cv::Sobel(image,sobelX,CV_16S,1,0,11);
	cv::Sobel(image,sobelY,CV_16S,0,1,11);
	cv::Mat sobel;
	//compute the L1 norm
